...
def get_i2c_write_latency():
    # Run the `time` command with `i2cset` and capture the output
    result = subprocess.run(
        ['time', 'sudo', 'i2cset', '-y', '0', '0x48', '0x22', '0x20'],  # Write 0x20 to address 0x22 on device 0x48
...
def test_i2c_write_latency():
    # Run the function to get the output and timings
    output, real_time, user_time, sys_time = get_i2c_write_latency()
    print("\nreal :",real_time,"\nuser :",user_time,"\nsys",sys_time)
    # Check that no output is expected (i2cset generally doesn't return anything)
